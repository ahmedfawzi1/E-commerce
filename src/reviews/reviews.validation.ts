...
class ReviewsValidation {
  ...
  createOne = [
    body("comment")
      ...
      .isLength({ min: 2, max: 150 }).withMessage((val, { req }) => req._('validation_length_medium')),

    body("rate")
      .notEmpty().withMessage((val, { req }) => req._('validation_field'))
      .isFloat({ min: 1, max: 5 }).withMessage((val, { req }) => req._('validation_value'))
  ...
  updateOne = [
    param("id").isMongoId().withMessage((val, { req }) => req._('invalid_id')).custom(async (val, { req }) => {
  ...
  deleteOne = [
    param('id')
      .isMongoId().withMessage((val, { req }) => req.__('invalid_id')).custom(async (val, { req }) => {
