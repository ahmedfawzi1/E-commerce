...
class SubCategoriesValidation {
    ...
    createOne = [
        ...
        body("description")
        .notEmpty().withMessage((val, {req})=> req.__('validation_field'))
        .isLength({ min: 2, max: 999 }).withMessage((val, {req})=> req.__('validation_length_long')),
        
        body("price")
        .notEmpty().withMessage((val, {req})=> req.__('validation_field'))
        .isFloat({ min: 2, max: 1000000 }).withMessage((val, {req})=> req.__('validation_length_long')),
        
        body("discount").optional()
        .isFloat({ min: 2, max: 100 }).withMessage((val, {req})=> req.__('validation_length_medium'))
        .custom(( val, {req} ) =>{
        ...
        .isInt({ min: 2, max: 1000 }).withMessage((val, {req})=> req.__('validation_length_long')),
        ...
        .custom(async (val: string, {req}) => {
          ...
          if(subCategory.category._id!.toString() !== req.body.category.toString()) throw new Error(`${req.__('not_found')}`)
          return true;
        }), validatorMiddleware 
     ];
    ...
    updateOne = [
        ...
        body("name").optional().isLength({ min: 2, max: 50 }).withMessage((val, {req})=> req.__('validation_length_short')),
